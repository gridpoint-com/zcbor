...
#define ZCBOR_WRITE_BYTE(state, byte) \
	do { \
		if (ZCBOR_IS_STREAMING(state)) { \
			uint8_t b = (byte); \
			size_t ret = state->constant_state->stream_write( \
...
static bool encode_indefinite_start(zcbor_state_t *state, uint8_t major_type)
{
	ZCBOR_CHECK_ERROR();
...
static bool encode_indefinite_end(zcbor_state_t *state)
{
	ZCBOR_CHECK_ERROR();
...
{
	return encode_indefinite_start(state, ZCBOR_MAJOR_TYPE_TSTR);
}

bool zcbor_bstr_start_encode_indefinite(zcbor_state_t *state)
{
	return encode_indefinite_start(state, ZCBOR_MAJOR_TYPE_BSTR);
}
...
{
	return encode_indefinite_end(state);
}
		...
		zcbor_stream_chunk_out call, void *ctx)
{
	...
	if (!zcbor_tstr_start_encode_indefinite(state)) {
		ZCBOR_FAIL();
	}
	...
	if (!zcbor_tstr_end_encode_indefinite(state)) {
		ZCBOR_FAIL();
	}
		...
		zcbor_stream_chunk_out call, void *ctx)
{
	...
	if (!zcbor_bstr_start_encode_indefinite(state)) {
		ZCBOR_FAIL();
	}
	...
	if (!zcbor_bstr_end_encode_indefinite(state)) {
		ZCBOR_FAIL();
	}
		...
		zcbor_stream_write_fn stream_write, void *stream_user_data, size_t elem_count)
{
	...
	if (state_array[0].constant_state) {
		...
		state_array[0].constant_state->stream_io = NULL;
	}
}

size_t zcbor_stream_bytes_written(const zcbor_state_t *state)
{
	if (!state || !ZCBOR_IS_STREAMING(state)) {
		return 0;
	}
